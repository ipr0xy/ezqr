...
export const CreateItem: React.FC = () => {
  ...
  const previewItem = useMemo((): Item | null => {
    if (!formData.name.trim() || !formData.upiId.trim() || !formData.price) {
      return null;
    }
    
    try {
      const price = parseAmount(formData.price);
      if (price <= 0) return null;
      
      const paymentNote = formData.paymentNote.trim() || generatePaymentNote(formData.name);
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              {previewItem ? (
                ...
                <div className="text-center py-8">
                  <div className="w-40 h-40 bg-white rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center mx-auto mb-4">
                  ...
                  <p className="text-gray-500 text-sm">
                    Fill in the required fields to generate your QR code
                  </p>
                  <p className="text-xs text-gray-400 mt-1">
                    Organisation name, UPI ID, and amount are required
                  </p>
                </div>
              )}
            </div>
